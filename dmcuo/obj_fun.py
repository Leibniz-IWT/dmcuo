   ...
   # #%% Model calculation for pure CuO, #1%Fe+CuO, #6%Fe+CuO and 1#0%Fe+CuO
    ...
    #    last_idx = find(data.time(l,:) ~= 0,1,'last')
     ...
     #   conc_eval{l} = conc_eval{l}(find(time_eval{l} <= opts.t_stop))
        ...
        V0 = c(1)*params.M_CuO/params.rho_CuO# % Particle volume in nm2
        ...
        c(5) = a0# % a_CuO - Particle surface area in nm2
        c(6) = V0# % V_CuO - Particle volume in nm2
       ...
       # #%% SOLUTION 1: Numerial solution of the model defined in reaction_kinetic_ODE_system using ode45
        ...
        scipy.integrate.solve_ivp(#radial_diffusion_PDE, t_span, y_0, method='LSODA',
       ...
       # #%% Save model results for plot
        ...
        if l == 1:
            pass#model.c_cu2p_2{l} = 0
        else:
            pass#model.c_cu2p_2{l} = deval(sol2,sol1.x)
